...
extension DrawerContentVC {
    ...
    func showMessageChatVC(_ message: CGMessage) {
        ...
        if Auth.auth().currentUser?.uid == nil {
            let alertMessage = "You must be connected to participate in this awsome conversation !"
            let alertController = UIAlertController(title: "Alert !", message: alertMessage, preferredStyle: .alert)
            
            let okAction = UIAlertAction(title: "Connect", style: .default, handler: { (action) in
                self.firebasePhoneConnect()
            })
        ...
        if let messageChatVC = UIStoryboard(name: "MessageChat", bundle: nil).instantiateInitialViewController() as? ChatContainerVC {
            messageChatVC.senderDisplayName = ""
            ...
            if let newMessageVC = newMessageNC.viewControllers[0] as? NewMessageVC {
                ...
                newMessageVC.messageAddedAction = { [weak self] locationAdded, message in
                    ...
                    if locationAdded != self?.currentLocation {
                        print("locationAdded != self?.currentLocation")
                        ...
                        if let parent = self?.parent as? PulleyViewController, let mapVC = parent.primaryContentViewController as? MapContentVC {
                            mapVC.callAPILocations(completion: { locations in
                                mapVC.selectAnnotation(location: locationAdded)
                                guard let index = locations.index(of: locationAdded) else { return }
                                self?.currentLocation = locations[index]
                            })
                        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isInSearchMode {
            ...
            if messages.count > indexPath.row {
                ...
                if let distance = message.postDistance {
                    if distance > 1000 {
                        cell.distanceLabel.text = "from \(String(format: "%.1f", distance/1000)) km"
                    }
                    else {
                        cell.distanceLabel.text = "from \(String(format: "%.0f", distance)) m"
                    }
                }
                else {
                    cell.distanceLabel.text = "from somewhere"
                }
